...
pub fn wl_subcompositor_handle_request(req: wlsc::Request,
                                       _: Main<wlsc::WlSubcompositor>)
{
    match req {
        wlsc::Request::GetSubsurface { id, surface, parent: par } => {
...
impl SubSurface {
    fn new(sub: Main<wlss::WlSubsurface>,
        ...
        match req {
            ...
            wlss::Request::PlaceAbove { sibling } =>
                self.ss_place_above = Some(
                    sibling.as_ref()
                        .user_data().get::<Rc<RefCell<Surface>>>()
                        .unwrap()
                        .borrow()
                        .s_id
                ),
