...
impl Display {
    ...
    unsafe fn select_surface_format(&self,
                                        pdev: vk::PhysicalDevice)
                                        -> vk::SurfaceFormatKHR
    {
...
pub struct Mesh {
    ...
    pool_handle: usize,
    sampler_descriptors: Vec<vk::DescriptorSet>,
}
...
impl Renderer {
    ...
    fn cbuf_record<F: FnOnce(&Renderer, vk::CommandBuffer)>
        (&self,
         cbuf: vk::CommandBuffer,
    ...
    pub fn record_next_cbuf(&mut self) {
        self.record_one_cbuf(self.current_image as usize);
    }

    // Record cbufs for all images
    //
    // usually we want to only record a cbuf for the next frame,
    // since the one after it might be different, but this helper
    // just does them all
    ...
    fn get_default_titlebar(&mut self) -> Titlebar {
    ...
    unsafe fn record_draw(rend: &Renderer,
                          cbuf: vk::CommandBuffer,
                          image_num: usize)
    {
